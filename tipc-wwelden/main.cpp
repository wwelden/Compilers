...
void usage(char *exe_name) {
  cerr <<"usage: " << exe_name << " [-hptd] [-v NUM] file" << endl;
  ...
  cerr << "\t-d            print debug information at verbocity 1" << endl; 
  cerr << "\t-v NUM        print debug information at verbocity NUM" << endl; 
}
...
void execute(char *exe_name, options &opts) {
  ifstream inputFile;
  if(!opts.pretty_print && !opts.table_print && ! opts.lex_print) opts.allocate = true;
  ...
  // Add another flag (--summary -s) to the options, and if it is set print the summary here. Be
  // sure to change main!
}
...
int main(int argc, char **argv) {
  ...
  while((ch = getopt_long(argc, argv, "dhlpatv:k:", longopts, NULL)) != -1) {
    ...
    switch(ch) {
      ...
      case 'v':
        ...
        break;
      case 'k':
        cout << "optarg: " << optarg << " atoi: " << atoi(optarg) << endl;
